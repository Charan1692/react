...
import { MdDelete, MdEdit, MdCheckBox } from "react-icons/md";
...
const Contact = ({ contact, contactKey }) => {
  ...
  const deleteContact = () => {
    ...
    firebase
      ...
      .remove()
      .then(() => toast("DELETED SUCEESSFULLY", { type: "warning" }))
      ...
      <Row>
        ...
        >
          <img
            src={contact.picture}
            alt=""
            className="img-circle profile border-secondary"
          />
        </Col>
        <Col md="8" onClick={() => viewSingleContact(contact)}>
          ...
          <div className="text-secondary">{contact.address}</div>
          <div className="text-secondary">
            <strong>{contact.cDateTime}</strong>
          </div>
        </Col>
        <Col
          md="1"
        ...
        >
          ...
          />
          <MdEdit
            className="icon text-warning ml-1"
